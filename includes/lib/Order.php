...
class Order
{
    ...
    public static function refund_info($trade_no, $api = 0, $uid = 0){
        ...
        $row = $DB->find('order', 'uid,api_trade_no,channel,money,realmoney,status', $where);
        if(!$row)
            return ['code'=>-1, 'msg'=>'当前订单不存在！'];
        ...
        return ['code'=>0, 'money'=>$row['realmoney']];
    }
    ...
    public static function refund($trade_no, $money, $api = 0, $uid = 0){
        ...
        $where = ['trade_no'=>$trade_no];
        if($uid > 0) $where['uid'] = $uid;
        $row = $DB->find('order', 'uid,channel,money,realmoney,getmoney,status', $where);
        if(!$row)
            return ['code'=>-1, 'msg'=>'当前订单不存在！'];
        if($row['status']!=1&&$row['status']!=3)
            return ['code'=>-1, 'msg'=>'只支持退款已支付状态的订单'];
        if($money>$row['realmoney']) return ['code'=>-1, 'msg'=>'退款金额不能大于订单金额'];
